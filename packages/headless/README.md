...
**@asakaa/headless** provides the business logic and state management for ASAKAA without any UI dependencies. Use these hooks with **React, Vue, Svelte, or Vanilla JavaScript** to build custom project management interfaces.
...
```bash
npm install @asakaa/headless @asakaa/core
```
...
```typescript
import { useCardDrag } from '@asakaa/headless'
...
```typescript
import { useMultiSelect } from '@asakaa/headless'
...
```typescript
import { useKeyboardNav } from '@asakaa/headless'
...
```tsx
import { useBoardState, useCardDrag } from '@asakaa/headless'
...
```
@asakaa/headless
├── uses → @asakaa/core (Pure TypeScript business logic)
└── provides → Framework-agnostic hooks

Your App (React/Vue/Svelte)
├── uses → @asakaa/headless
...
- **[@asakaa/core](../core)** - Pure TypeScript business logic
- **[@asakaa/board](../board)** - React UI components
- **[@asakaa/react](../react)** - React-specific hooks (coming soon)
