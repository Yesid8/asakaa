...
import { BoardStore, type BoardState, type CardData, type ColumnData, type BoardData } from '@asakaa/core'
...
export function BoardProvider({ children, initialData, onStateChange }: BoardProviderProps) {
  ...
  if (!storeRef.current) {
    ...
    if (initialData?.board) {
      const { Board } = require('@asakaa/core')
