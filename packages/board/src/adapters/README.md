# React Adapters for @asakaa/core

Framework adapters that connect the framework-agnostic `@asakaa/core` with React.
...
```tsx
import { BoardProvider, useBoard, useFilteredCards } from '@asakaa/board/adapters/react'
...
import { cardsAtom } from '@asakaa/board'
...
**After (v0.7.0 - Core)**:
```tsx
import { useBoard } from '@asakaa/board/adapters/react'
...
Uses `@asakaa/core` under the hood for all state management.
