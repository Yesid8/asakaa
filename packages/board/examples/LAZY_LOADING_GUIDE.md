# Lazy Loading Guide - @asakaa/board

## Overview

Starting with v0.7.0, @asakaa/board supports code splitting for optimal bundle size. Heavy components are lazy-loaded on demand, reducing initial bundle from ~254KB to ~80KB (70% reduction).
...
import { KanbanBoard, Card, Column } from '@asakaa/board'
...
```tsx
import { VelocityChart, BurnDownChart, DistributionCharts } from '@asakaa/board/lazy'
```

### PDF Export - ~150KB
```tsx
import { ExportImportModal } from '@asakaa/board/lazy'
```

### Modals - ~30KB
```tsx
import { CardDetailModal, CardDetailModalV2 } from '@asakaa/board/lazy'
```
...
```tsx
import { GeneratePlanModal, AIUsageDashboard } from '@asakaa/board/lazy'
```
...
const VelocityChart = dynamic(
  () => import('@asakaa/board/lazy').then(m => m.VelocityChart),
